...
, stats_timer(ioc)
{
   ...
   redis_pub_session.send(std::move(r));
   do_stats_logger();
}
                                 ...
                                 , redis_req const& req)
{
   ...
   if (req.cmd == redis_cmd::get) {
      ...
      redis_req r
      { redis_cmd::subscribe
      , gen_resp_cmd(redis_cmd::subscribe, {redis_group_channel})
      , ""
      };
      redis_gsub_session.send(std::move(r));
...
ev_res server_mgr::on_login(json j, std::shared_ptr<server_session> s)
{
   ...
   redis_ksub_session.send(std::move(r));
                      ...
                      , std::shared_ptr<server_session> s)
{
   ...
   redis_req r
   { redis_cmd::publish
   , gen_resp_cmd(redis_cmd::publish, { redis_group_channel, msg})
   , ""
   };
